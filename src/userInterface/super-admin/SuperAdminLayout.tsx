...
import { LayoutDashboard, Bell, Inbox, Check } from 'lucide-react';
...
const SuperAdminLayout: React.FC = () => {
    ...
    const [readNotifications, setReadNotifications] = React.useState<NotificationItem[]>([]);
    const [isNotifOpen, setIsNotifOpen] = React.useState(false);
    const [activeTab, setActiveTab] = React.useState<'unread' | 'read'>('unread');
    const [page, setPage] = React.useState(1);
    const [hasMore, setHasMore] = React.useState(true);
    const [isLoadingRead, setIsLoadingRead] = React.useState(false);
    ...
    const fetchReadNotifications = React.useCallback(async (pageNum: number, append: boolean = false) => {
    ...
    React.useEffect(() => {
        ...
        const onNotification = (data: NotificationItem) => {
            console.log("Live notification received on frontend:", data);
        ...
        <div className="h-screen bg-black flex font-sans overflow-hidden w-full">
            ...
            <main className="flex-1 flex flex-col h-full bg-black text-foreground relative overflow-hidden">
                ...
                <div className="hidden lg:flex items-center justify-between p-4 px-6 border-b border-zinc-800 bg-black z-30 flex-shrink-0">
                    <div className="text-sm font-semibold tracking-widest text-zinc-400">ADMIN PORTAL</div>
